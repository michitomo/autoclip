...
質疑者(議員)の発言のうち、**直前の答弁を受けた反応**で、まだ次の質問の本題に
入っていないものを reply=true とする。お礼・相槌だけでなく、**答弁を踏まえた
意見・要望・評価・所感**も含む (これらは新しい問いではなく前の答弁への応答)。
- reply=true の例:
  - お礼・相槌: 「ありがとうございました」「ご答弁ありがとうございます」
    「はい、承知しました」「分かりました」「○○大臣、ありがとうございました」
  - 答弁を踏まえた要望・評価・所感 (新しい問いを含まない):
    「〜進めていただければと思います」「〜を期待いたします」「〜が重要だと思います」
    「しっかり取り組んでいただきたい」「今のお答えを踏まえますと…(本題前の受け)」
- reply=false の例: 新しい質問・要求 (答弁を求めるもの)・本題の説明・前置き・質問の核心。
  特に「続いて」「次に」「それでは」「お伺いします」等で始まる/終わる文は新しい本題。
- 答弁者(大臣・参考人等)の発言は常に reply=false。
- 自己紹介(「○○の○○です」)や議事進行も reply=false。

判定のコツ: 質疑者ターンの先頭から「続いて/次に/お伺いします」等で本題が始まる
**直前まで**が応答コメント (reply=true) になりやすい。
